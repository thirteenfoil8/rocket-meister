...
class Agent():
    ...
    def __init__(self):
        ...
        self.downsample = nn.Sequential(
            nn.Conv1d(10, 16, kernel_size=1, stride=1, bias=False))
        self.net = Net(self.downsample).double().to(device)
...
if __name__ == "__main__":
    ...
    for i_ep in range(100000):
        ...
        for t in range(10000):
            ...
            #* np.array([0.1,2.]) + np.array([-0.05, -1.])
            ...
            score += reward
            state = state_
        df = df.append(pd.Series(data=[i_ep,score,running_score,'PPO']),ignore_index=True)
        ...
        if i_ep % 10 == 0:
            ...
            agent.save_param()
    df.columns = ['i_ep','score','running_score','Algorithm']
